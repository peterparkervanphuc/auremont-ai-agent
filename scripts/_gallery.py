...
Original images live on R2/MinIO and are never checked into git, so a fresh clone
has the manifest but no local image directory to scan.
"""
...
def gallery_by_slug() -> dict[str, list[str]]:
    ...
    bucket = get_settings().minio_bucket_project_images
    grouped: dict[str, list[str]] = {}
    for key in json.loads(MANIFEST_PATH.read_text(encoding="utf-8")).get("images", []):
        slug, _, filename = key.partition("/")
        if filename:
            grouped.setdefault(slug, []).append(public_object_url(bucket, key))
    return grouped
